...
<div class="content">
  ...
  ${coach ? '<div class="card"><div class="card-title">Your Coach</div><div class="coach-wrap">' + (coach.avatar_url ? '<img class="avatar" src="' + coach.avatar_url + '" alt="' + (coach.full_name||'Coach') + '" onerror="this.remove()">' : '<div class="avatar-ph">🏆</div>') + '<div><div class="coach-name">' + (coach.full_name||'Coach') + '</div>' + (coach.coach_rating ? '<div class="stars">' + renderStars(coach.coach_rating) + ' ' + coach.coach_rating.toFixed(1) + (coach.coach_rating_count ? ' <span style="color:#6B6F7E;font-size:12px;">(' + coach.coach_rating_count + ' reviews)</span>' : '') + '</div>' : '') + (coach.coach_license ? '<div class="coach-sub">📋 ' + coach.coach_license + '</div>' : '') + (coach.instagram_handle ? '<div class="coach-sub">📷 @' + coach.instagram_handle + '</div>' : '') + '</div></div>' + (coach.coach_bio ? '<div class="bio">' + coach.coach_bio + '</div>' : '') + (coach.coach_specialities && coach.coach_specialities.length > 0 ? '<div class="spec-wrap"><div class="lbl" style="margin-bottom:6px;">Specialities</div><div class="tags">' + (Array.isArray(coach.coach_specialities)?coach.coach_specialities:[coach.coach_specialities]).map(s=>'<span class="tag hi">'+s+'</span>').join('') + '</div></div>' : '') + '</div>' : ''}
